...
The expected behavior is not clearly defined yet.  
...
This test is blocked by BUG-001.  

The Hammer category filter currently returns no products, so the combined search and category filter behavior cannot be verified.

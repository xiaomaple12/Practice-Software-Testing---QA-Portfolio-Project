...
| SQL-VAL-SEARCH-005 | TC-SEARCH-005 | Verify that pressing Enter triggers the same search result as clicking the Search button. | `SELECT * FROM products WHERE LOWER(name) LIKE '%hammer%';` | The query should return 7 Hammer-related product records, including Sledgehammer. |
| SQL-VAL-SEARCH-006 | TC-SEARCH-007 | Verify that search results are case-insensitive. | `SELECT * FROM products WHERE LOWER(name) LIKE '%hammer%';` | The query should return the same 7 Hammer-related product records for both `Hammer` and `hammer`, including Sledgehammer. |
...
| SQL-VAL-SEARCH-010 | TC-SEARCH-012 | Verify that the Sledgehammer product record exists in the database and inspect whether its product data is valid. | `SELECT * FROM products WHERE LOWER(name) = 'sledgehammer';` | The query should return one Sledgehammer product record. The record should contain valid product information and should not be inactive, disabled, or deleted. |
| SQL-VAL-SEARCH-011 | TC-SEARCH-012 | Verify that a case-insensitive partial-match query for `hammer` returns all matching products, including Sledgehammer. | `SELECT * FROM products WHERE LOWER(name) LIKE '%hammer%';` | The query should return 7 matching product records, including Sledgehammer. This would differ from the current Search API response total of 6. |
| SQL-VAL-SEARCH-012 | TC-SEARCH-013 | Verify that Sledgehammer is correctly linked to the Hammer category in the database. | `SELECT p.*, c.name AS category_name FROM products p JOIN categories c ON p.category_id = c.id WHERE LOWER(p.name) = 'sledgehammer';` | The query should return one Sledgehammer product record linked to the Hammer category. |

...
| SQL-VAL-SEARCH-008 | TC-SEARCH-010 | Verify whether Hammer category has product records in the database. | `SELECT p.\* FROM products p JOIN categories c ON p.category\_id = c.id WHERE c.name = 'Hammer';` | The query should return Hammer-related product records if the Hammer category is correctly linked to products. |
...
It can be used to verify search result counts.



It can help confirm whether no-result behavior is consistent with product data.



It can also support bug investigation by checking whether an issue may come from data, category mapping, API filtering, or UI behavior.
